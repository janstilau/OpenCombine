...
extension Publisher {
    ...
    /// a given predicate.
    // 从上面的描述来看, 这一定是一个, 只有上游节点 Complete 的时候, 才会给下游节点, 发送 Bool 值的一个 Operator
    ...
    // 这里有一个剪枝的操作, 如果提前知道了不可能完全满足, 那么可以提前给后续的节点, 发送 false 数据.
    ...
    // 这里说的很明白, 只要是判断为 false 了, 直接对上游进行 cancel. 然后发送 False 给下游节点.
    ...
    /// If the predicate throws an error, the publisher fails and
    /// passes the error to its downstream subscriber.
    // 如果, Predicate 闭包 Throw 了错误, 那么直接将捕捉的错误, 传递给后方.
...
extension Publishers {
    ...
    // AllSatisfy 真正的 Publisher. 主要做的就是信息收集的工作.
    ...
    public struct AllSatisfy<Upstream: Publisher>: Publisher {
        ...
        // 惯例, Operator 存储上游节点, 在真正建立响应链条的时候使用.
        ...
        // 惯例, 使用 Operator 记录的各种数据, 来生成节点. 然后使用记录的 upstream 对节点进行 attach.
...
extension Publishers.TryAllSatisfy {
    ...
    // 下游节点, 不在是上游节点的错误类型, 而是回归到了 Error 通用数据类型. 
