extension Result {
    ...
    public struct OCombine {
        ...
        /*
         一个发布者（publisher），它向每个订阅者发送一次输出，然后完成，或者立即失败而不产生任何元素。

         如果 result 是 .success，那么 Once 会等待至少请求1个值，然后才发送输出。如果 result 是 .failure，那么 Once 在订阅时立即发送失败。

         与 Just 不同，Once 发布者可以以错误终止而不是发送值。与 Optional 不同，Once 发布者始终发送一个值（除非以错误终止）
        */
