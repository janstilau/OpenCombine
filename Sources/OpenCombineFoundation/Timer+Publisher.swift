...
extension Foundation.Timer {
    ...
    public enum OCombine {
        ...
        public final class TimerPublisher: ConnectablePublisher {
            ...
            /*
             对于 ConnectablePublisher 来说
             receive<Downstream: Subscriber> 只是进行下游节点的记录.
             而真正的上游节点信号产生逻辑, 要在 connect 的时候, 才真正的触发.
             */
            ...
            public func connect() -> Cancellable {
                ...
                // 这里有强引用, 因为 Block 直接使用 handleTimerTimeout, 会进行 self 的引用
                let timer = Timer(timeInterval: interval, repeats: true, block: handleTimerTimeout)
            ...
            private func handleTimerTimeout(_ timer: Timer) {
                lock.lock()
            ...
            where Downstream.Input == Date, Downstream.Failure == Never
            {
                ...
                // 这是一个 Publisher, 所以没有 receive output 这样的函数.
